...
#indicate it is the code for alternative case
case <- "alternative"
...
#set the sample size for estimating the true IIE
n_true <- 5000
...
n_M <- 134
set.seed(1) # make sure the parameters are fixed in all replications
eta_0 <- 4
theta_0 <- -1
theta_z <- 2.5
theta_x <- c(-0.8, -0.2)
...
#estimate the true value of IIE under alternative case
wrapper0 <- function(i){
  seed <- i
  set.seed(seed)
  X0 <- sample_n(realX, size = n_true, replace = TRUE)
  X <- matrix(as.numeric(as.matrix(X0)), ncol=2)
  X[,1] <- X[,1]/100
  
  IIE.true <- IIE.IPW.true(n_true, seed, X, alpha_0, alpha_x, gamma_0, gamma_z, gamma_x, eta_0, theta_0, theta_z, theta_x, theta_l, exp_order, eta_lower, eta_upper, 
...
library(snowfall)
sfInit(parallel = TRUE, cpus = ncpus)  #the number of cpus can be chosen based on the characteristics of cluster
...
outputs.true <- sfLapply(1:nrep, wrapper0)

IIE_true_all <- rep(0, nrep)
for (j in 1:nrep){
  IIE_true_all[j] <- outputs.true[[j]]
}

IIE_true_all2 <- IIE_true_all[(IIE_true_all<(1+10^(-6)))&(IIE_true_all>(-1-10^(-6)))]
IIE.true <- mean(IIE_true_all2, na.rm=TRUE)


#estimate the IIE and its confidence interval
wrapper <- function(s){
  ...
  while((condition1| condition2| condition3) & (counter < iter1)){
    ...
    for (j in 1:n_M){
      ...
      a <- tryCatch({
        ...
        model <- tune.fit(x, y, family = 'binomial', penalty = penalty,  type.measure = "deviance", tune = tune, nfolds = 5, penalty.factor = penalty.factor0)
...
IIE_tbl <- 
  ...
  mutate(Normal.lower = IIE_hat + qnorm(0.025)*sd,
         ...
         Normal.correct = (Normal.lower < 0 & Normal.upper > 0)) %>% 
...
#save the results 
save(true.parameters, IIE.true, IIE_hat_all, IIE_hat_bs_all, IIE_bias, IIE_sd, IIE_rmse, power, file = paste("./results_penalty_", penalty, "_tune_", tune, "_case", case, "_n", n_subj, "_nrep", nrep, ".RData", sep=""))
